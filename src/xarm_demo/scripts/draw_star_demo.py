...
class DrawStarDemo:
    def __init__(self):
        ...
        # 初始化ROS节点
        rospy.init_node('draw_star_demo', anonymous=True)
        ...
        star_pose.pose.position.x = 0.48
        star_pose.pose.position.y = -0.20
        star_pose.pose.position.z = 0.10
        ...
        # 向下放笔
        star_pose.pose.position.z = 0.069
        arm.set_pose_target(star_pose, end_effector_link)
        arm.go()

        radius  = 0.08
        centerY = star_pose.pose.position.y
        centerX = star_pose.pose.position.x - radius
        ...
        for th in numpy.arange(0, 6.2831855, 1.2566371):
            ...
            pose_temp.pose.position.z = star_pose.pose.position.z
        ...
        end_pose.header.frame_id = reference_frame
        end_pose.header.stamp = rospy.Time.now()     
        end_pose.pose = arm.get_current_pose(end_effector_link).pose
        end_pose.pose.position.z += 0.01

        arm.set_pose_target(end_pose, end_effector_link)
        arm.go()
        rospy.sleep(1)
